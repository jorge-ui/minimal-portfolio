...
window.testImageUpload = () => {
	async function setScreenShots(name: string) {
		const screenShotObj = {
			screenshots: {
				desktop: {},
				mobile: {}
		...
		const buckedRef = fire_storage.ref(`projects-screenshots/${name}`);

		const listResult = await buckedRef.list();

		for (let prefixRef of listResult.prefixes) {
			let prefixListResult = await prefixRef.list();

			for (let itemRef of prefixListResult.items) {
				const downloadURL = await itemRef.getDownloadURL();
				addDownloadUrl(downloadURL)
			}
			function addDownloadUrl(downloadUrl: string) {
				let toAddObj = screenShotObj.screenshots[prefixRef.name as "desktop" | "mobile"] as object;
				// @ts-ignore
				toAddObj[Object.keys(toAddObj).length] = downloadUrl
			}
		}

		console.log(screenShotObj);

		firestore.collection('projects').doc(name).set(screenShotObj, {
			merge: true
		}).then(() => {
			console.log(`successfully updated ${name} screenshots`);
		})
	}

	void setScreenShots('movie-ratings')

	// void setScreenShots('burger-builder')

	// fire_storage
	// 	.ref('projects-screenshots/burger-builder')
	// 	.listAll()
	// 	.then(data => {
...
// @ts-ignore
window.testWriteProjects = () => {
	projectsData.forEach(project => {
		delete project.icon;
		delete project.screenshots;
		cleanSummary(project)
		firestore.collection('projects').doc(project.name).set(project)
			.then(done => console.log(`done`, {done}))
	})
}

function cleanSummary(project: ProjectsData) {
	let clean = project.summary.replace(/\n/g, ' ').replace(/( )( *)/g, (match, p1) => p1)

	if (clean[0] === ' ')
		project.summary = clean.substr(1)
	else project.summary = clean;
}
